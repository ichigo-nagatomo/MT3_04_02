...
const char kWindowTitle[] = "LD2B_06_ナガトモイチゴ_MT3_04_02";
...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	pendulum.angle = 0.7f;
	pendulum.angularVelo = 0.0f;
	pendulum.angularAcceleration = 0.0f;

	Vec3 ball = {0.0f, 0.0f, 0.0f};
	...
	while (Novice::ProcessMessage() == 0) {
		...
		if (play) {
			...
			pendulum.angularVelo += pendulum.angularAcceleration * deltaTime;
			pendulum.angle += pendulum.angularVelo * deltaTime;
		}

		ball.x = pendulum.anchor.x + std::sin(pendulum.angle) * pendulum.length;
		ball.y = pendulum.anchor.y - std::cos(pendulum.angle) * pendulum.length;
		ball.z = pendulum.anchor.z;
		...
		DrawLine({pendulum.anchor, ball / 2.0f} , projectionMatrix , viewportMatrix , WHITE);
